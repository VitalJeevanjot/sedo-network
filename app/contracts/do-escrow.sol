...
contract DomainOffering is ChainlinkClient {
    ...
    // bytes32 public recentWhoisResponse;
    ...
    function fulfillWhois(bytes32 _requestId, string memory whois_email) public recordChainlinkFulfillment(_requestId)
    {
        // TO RUN this function required String value from chainlink adapter
        // recentWhoisResponse = whois_email;
        ...
        recentBuyerEmailHash = keccak256(abi.encodePacked(email_to_match));
        recentWhoisHash = keccak256(abi.encodePacked(whois_email));
        if(recentWhoisHash == recentBuyerEmailHash) {
            msg.sender.transfer(1000000000000000000); // right now this value says no fee but can be modified later...
            entity[_domainName].Current_Self_Claimed_Owner.transfer(entity[_domainName].Amount_Buyer_Paid_For_It - 0); // right now this value says no fee but can be modified later...
